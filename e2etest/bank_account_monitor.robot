...
Sort skasse account statement by sort rule into csv file
    ...
    And Two sort rules
    When Bank account monitor is called with skassen account statement
    Then A Csv file with sorted skassen transactions where created

Sort VR bank account statement by sort rule into csv file
    Given A VR bank account statement with three transactions
    And Two sort rules
    When Bank account monitor is called with vrbank account statement
    Then A Csv file with sorted vrbank transactions where created
...
Sort all skassen and vr bank transactions within a directory
    ...
    And Two sort rules
    And Some other file
    When Bank account monitor is called with directory of account statements
    Then A Csv file with sorted skassen and vrbank transactions where created
...
Sort VR bank account statement into csv file and show savings
    ...
    Then Created Csv file should be equal to    ${SORTED_VRBANK_TRANS_2}
...
Exception for invalid bank statement file is logged
    When Bank account monitor is called with wrong account statement path
    Then Error is logged to standard err    ${EXCEPTION_INVALID_BANK_STATEMENT}

Exception for invalid sort rule file is logged
    Given A skasse bank statement with three transactions
    When Bank account monitor is called with wrong sort rule path
...
A skasse bank statement with three transactions
    ...
    ...    ${EXAMPLE_SKASSE_ACCOUNT_STATEMENT}

A VR bank account statement with three transactions
    Create File    ${VR_BANK_STATEMENT_FILE}
...
Some PDF file without transactions
    PdfCreator.Write    ${SKASSE_BANK_STATEMENT_FILE}
    ...    "Skassen Hello my name is Emil"
...
Bank account monitor is called with skassen account statement
    Generic bank account monitor call
    ...    ${SKASSE_BANK_STATEMENT_FILE}
    ...    ${SORT_RULE_FILE}
    ...    ${True}
    ...    ${CSV_OUTPUT_FILE}
...
Bank account monitor is called with vrbank account statement
    Generic bank account monitor call
...
Bank account monitor is called with directory of account statements
    Generic bank account monitor call
    ...    ${ANALYZE_DIR}
    ...    ${SORT_RULE_FILE}
    ...    ${True}
...
Bank account monitor is called with wrong account statement path
    Generic bank account monitor call
    ...    /some/path.txt
    ...    ${SORT_RULE_FILE}
    ...    ${True}
...
Bank account monitor is called with wrong sort rule path
    Generic bank account monitor call
    ...    ${SKASSE_BANK_STATEMENT_FILE}
    ...    /some/path.txt
    ...    ${True}
...
Bank account monitor is called with invalid cvs presenter output path
    Generic bank account monitor call
    ...    ${SKASSE_BANK_STATEMENT_FILE}
    ...    ${SORT_RULE_FILE}
    ...    ${True}
...
A Csv file with sorted skassen transactions where created
    Created Csv file should be equal to    ${SORTED_SKASSEN_TRANS}

A Csv file with sorted vrbank transactions where created
    Created Csv file should be equal to    ${SORTED_VRBANK_TRANS}

A Csv file with sorted skassen and vrbank transactions where created
    Created Csv file should be equal to    ${SORTED_SKASSEN_VRBANK_TRANS}

Created Csv file should be equal to
