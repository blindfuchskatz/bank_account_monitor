...
class ACsvPresenter(unittest.TestCase):
    ...
    def testWriteEmptyMiscDict(self):
        self.file_checker.dir_of_file_exists = MagicMock(return_value=True)
        self.file_writer.write = MagicMock()
        cp = CsvPresenter(self.file_checker, self.file_writer, SOME_PATH)
    ...
    def testRaiseExceptionWhenOutputPathIsMissing(self):
        ...
        self.ca.assertRaisesWithMessage(
            NO_PATH, CsvPresenter, self.file_checker, self.file_writer, "")

    def testRaiseExceptionWhenOutputPathIsInvalid(self):
        self.file_checker.dir_of_file_exists = MagicMock(return_value=False)
        self.ca.assertRaisesWithMessage(
            INVALID_INPUT_PATH.format(SOME_PATH), CsvPresenter, self.file_checker, self.file_writer, SOME_PATH)
